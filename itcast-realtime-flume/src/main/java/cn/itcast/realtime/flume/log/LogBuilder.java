...
public class LogBuilder {
    private static final Logger logger =  Logger.getLogger("msg");

    public static void main(String[] args) {
        Random random = new Random();
        List list = logInfoList();
        while (true) {
            try {
                logger.error(list.get(random.nextInt(list.size())));
                Thread.sleep(2000);
    ...
    private static List<String> logInfoList() {
        ...
        list.add("msg:error: Caused by: java.lang.NoClassDefFoundError: com/starit/gejie/dao/SysNameDao");
