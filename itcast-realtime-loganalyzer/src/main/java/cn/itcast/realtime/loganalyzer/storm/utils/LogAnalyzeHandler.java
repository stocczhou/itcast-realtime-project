package cn.itcast.realtime.loganalyzer.storm.utils;


import cn.itcast.realtime.loganalyzer.storm.constant.LogTypeConstant;
...
public class LogAnalyzeHandler {
    ...
    public static void process(LogMessage logMessage) {
        //加载所有的Task ---->jobMap static 初始化过一次。程序运行起来之后，已经有值。
        if (jobMap == null || jobDetail == null) {
            loadDataModel();
        }
        // 消息分为不同的类型、产品配置任务的时候，是针对不对类型的数据配置的。
        // 如果当前数据是一条浏览类型的数据，只需要获取浏览类型相关的Job
        ...
        for (LogAnalyzeJob logAnalyzeJob : analyzeJobList) {
            ...
            // 这里面的LogAnalyzeJobDetail 就是我们一直在说的condition条件。
            ...
            for (LogAnalyzeJobDetail jobDetail : logAnalyzeJobDetailList) {
                // 比较字段名称、比较方式、比较值
                // jobDetail.getField() 是获取condition中要比较的字段。接着从KafkaSpout发送过来的数据中获取字段对应的值。
            ...
            //当一个Task/Job所有的condition都被满足之后。
            //对数据进行赋值和修改。
            if (isMatch) {
                //设置pv  口径：来一个一个。string
                ...
                String ipKey = "log:" + logAnalyzeJob.getJobName() + ":ip:" + DateUtils.getDate();
